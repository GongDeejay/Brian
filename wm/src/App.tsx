...
export default function App() {
  ...
  const handleSaveNBack = (result: NBackResult) => {
    applyOutcome(saveNBackResult(result));
  };

  const handleSaveOSPAN = (result: OSPANResult) => {
    applyOutcome(saveOSPANResult(result));
  };

  const handleSaveChangeDetection = (result: ChangeDetectionResult) => {
    applyOutcome(saveChangeDetectionResult(result));
  };
  ...
  const handleClearHistory = useCallback(() => {
    ...
    applyOutcome(clearCognitiveHistory());
  ...
  const handleUndoClear = useCallback(() => {
    ...
    applyOutcome(persistCognitiveProfile(undoSnapshot));
    setUndoSnapshot(null);
  }, [undoSnapshot]);
      ...
      {storageNotice && (
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex items-start gap-2 text-xs">
            ...
            <span className="flex-1">{storageNotice}（结果已保留在当前页面并可在结果页导出）</span>
            <button
